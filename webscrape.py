...
class StadtReutlingenSource(WeatherSource):
    ...
    def get_high_low(self):
        ...
        high_value = int(''.join(ch for ch in high_span.get_text(strip=True) if ch.isdigit()))
        low_value = int(''.join(ch for ch in low_span.get_text(strip=True) if ch.isdigit()))
...
class WetterNetSource(WeatherSource):
    ...
    def get_high_low(self) :
        ...
        temp_tags = soup.find_all('h2',class_=['white', 'center', 'tempText', 'todayText'])  # <-- same for high & low now
        ...
        if high_tag is None or low_tag is None:
            raise ValueError('Could not find high and low temperature tags in wetter.net HTML.')

        # extract the text and keep only the digits
        high_value = int(''.join(ch for ch in high_tag.get_text(strip=True) if ch.isdigit()))
        low_value = int(''.join(ch for ch in low_tag.get_text(strip=True) if ch.isdigit()))
